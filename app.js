    const DEFAULT_CONFIG = {
      ...
      CARD_FONT_SCALE: 100,
      SEVERITY_COLORS: window.IncidentTheme.DEFAULT_SEVERITY_COLORS,
    ...
    function applyPanelCustomization() {
      ...
      const severityColors = window.IncidentTheme.normalizeSeverityColors(state.config.SEVERITY_COLORS);
      const rootStyle = document.documentElement.style;

      state.config.DASHBOARD_THEME = window.IncidentTheme.apply(theme);
      state.config.SEVERITY_COLORS = severityColors;
      Object.entries(severityColors).forEach(([severity, color]) => {
        const variable = severityCssVariable[severity];
        rootStyle.setProperty(`--${variable}`, color);
        rootStyle.setProperty(`--${variable}-text`, window.IncidentTheme.contrastColor(color));
      });
    ...
    function applyDemoSettings(renderChanges = false) {
      ...
      try {
        ...
        state.config = {
          ...
          SEVERITY_COLORS: saved.severity_colors || state.config.SEVERITY_COLORS,
          ...activeFontPreview,
        };
      } catch {
