...
function renderSummary(problems) {
  const activeProblems = problems.filter(isActiveIncident);
  const resolvedProblems = problems.filter(problem => !isActiveIncident(problem));
  const counts = countBySeverity(activeProblems);
  const activeTotal = activeProblems.length;

  elements.totalProblems.textContent = activeTotal;
  ...
  elements.totalCard.classList.toggle("alerting", activeTotal > 0);
  updateTotalCardNote(problems);
  ...
  elements.statusPill.textContent = activeTotal > 0 ? "Em alerta" : "Operacional";
  elements.statusPill.classList.toggle("alerting", activeTotal > 0);
  ...
  const resolvedText = resolvedProblems.length > 0 ? ` - ${resolvedProblems.length} resolvido(s) recente(s)` : "";
  elements.panelSubtitle.textContent = activeTotal === 1
    ? `1 incidente ativo${resolvedText} - periodo: ${currentPeriodLabel} - ordenando por ${sortLabel}`
    : `${activeTotal} incidentes ativos${resolvedText} - periodo: ${currentPeriodLabel} - ordenando por ${sortLabel}`;
}
...
function updateTotalCardNote(problems) {
  if (!elements.totalNote) return;

  const activeProblems = (problems || []).filter(isActiveIncident);
  const resolvedProblems = (problems || []).filter(problem => !isActiveIncident(problem));
  const activeTotal = activeProblems.length;
  const affectedClients = new Set(activeProblems.map(problem => problem.clientName).filter(Boolean)).size;
  const longestProblem = [...activeProblems].sort((a, b) => getProblemDurationSeconds(b) - getProblemDurationSeconds(a))[0];
  const longestDuration = longestProblem ? formatDuration(longestProblem.clock, longestProblem.rClock) : "sem incidente";
  const notes = [
    `Total de incidentes ativos: ${activeTotal}`,
    `Clientes afetados: ${affectedClients}`,
    `Maior duracao: ${longestDuration}`,
    `Resolvidos recentes: ${resolvedProblems.length}`
  ];
  const index = Math.floor(Date.now() / 5000) % notes.length;
  elements.totalNote.textContent = notes[index];
}

function isActiveIncident(problem) {
  return problem?.status !== "RESOLVIDO";
}

function isNewIncident(problem) {
  if (!isActiveIncident(problem)) return false;

  const clock = Number(problem.clock) || 0;
  const ageSeconds = Math.floor(Date.now() / 1000) - clock;
  ...
  elements.problemList.innerHTML = problems.map(problem => `
    <div class="problem-row ${problem.status === "RESOLVIDO" ? "resolved" : "incident"}">
      ...
      <div class="problem-cell">
        <div class="problem-name" title="${escapeHtml(problem.name)}">${isNewIncident(problem) ? '<span class="new-badge">NOVO</span>' : ""}${escapeHtml(problem.name)}</div>
...
function calculateRowHeight(rowCount) {
  ...
  return Math.max(78, Math.floor(panel.height / effectiveRows));
}
...
function updateClock() {
  ...
  updateTotalCardNote(state.problems);
